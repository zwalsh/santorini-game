...
describe("JsonToComponent", function () {
  ...
  describe("createBuildAction", function () {
    let gameState;
    let workerNameToId;
    let buildAction;
    beforeEach(function () {
      ...
      gameState = createdGame[0];
      workerNameToId = createdGame[1];
      let moveAction = JsonToComponent.createMoveAction(["move", "b1", ["PUT", "SOUTH"]], workerNameToId, gameState);
      Action.execute(moveAction, gameState);
      let buildActionInput = ["+build", ["EAST", "PUT"]];
      buildAction = JsonToComponent.createBuildAction(buildActionInput, workerNameToId.get("b1"), gameState);
    });
    it("uses the correct WorkerId", function () {
      expect(buildAction.getWorkerId()).to.eql(2);
    });
    it("uses the correct location", function () {
      expect(buildAction.getLoc()).to.eql([1, 3]);
    });
    it("creates the correct type of Action", function () {
      expect(buildAction.getType()).to.eql(Action.BUILD);
    });
  });
  describe("getLocation", function () {
    it("provides the correct adjacent location", function () {
      let board = new Board();
      let workerId = board.addWorker(3, 3);
      let location = JsonToComponent.getLocation(board, workerId, Direction.EAST);
      expect(location).to.eql([3, 4]);
    });
  });
});
